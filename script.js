...
function loadCallDataForTelecaller(telecallerEmail) {
    console.log('Loading data for telecaller in script.js:', telecallerEmail);
    database.ref('calls').orderByChild('telecallerEmail').equalTo(telecallerEmail).once('value')
        .then((snapshot) => {
            ...
            if (data) {
                ...
                console.log('No data found for telecaller:', telecallerEmail);
                callData.details = [];
                updateDashboardUI(callData);
                updateSummaryNumbers();
            }
...
// Modify the existing updateDashboardUI function if it doesn't exist
...
window.addEventListener('message', function(event) {
    ...
    if (event.data.type === 'newCall' || event.data.type === 'callUpdated') {
        ...
        updateDashboard();
    } else if (event.data.type === 'callRemoved') {
        updateDashboardWithRemovedCall(event.data.call);
    } else if (event.data.type === 'callUpdated') {
        updateDashboardWithNewCall(event.data.call);
    }
});
